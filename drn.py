...
class aspp(nn.Module):
    # config = [in_channel_0,out_channel_0,rate_0]
    def __init__(self,config):
        ...
        for con in config:
             if con[2] == 1:
                 ...
                 k = 3 
                 pad = con[2]
             #self.collection.append(nn.Sequential(nn.Conv2d(con[0],con[1],kernel_size=k,stride=1,padding=pad,dilation=con[2],bias=False),
             #                            nn.BatchNorm2d(con[1]),
             #                            nn.ReLU(inplace=True)))
             self.collection.append(nn.Sequential(SeparableConv2d(con[0],con[1],kernel_size=k,stride=1,padding=pad,dilation=con[2],bias=False),
                                         nn.BatchNorm2d(con[1])
                                          ))
        #self.global_avg_pool = nn.Sequential(nn.AdaptiveAvgPool2d((1, 1)),
        #                                     nn.Conv2d(con[0], 512, 1, stride=1, bias=False),
        #                                     nn.BatchNorm2d(512),
        #                                     nn.ReLU(inplace=True))
        self.global_avg_pool = nn.Sequential(nn.AdaptiveAvgPool2d((1, 1)),
        ...
        self.conv_out = SeparableConv2d(4*con[1],con[1],1,stride=1,bias=False)
    ...
    def __init__(self, block, layers, num_classes=1000,
        ...
        if arch == 'C':
            ...
            self.layer0 = nn.Sequential(
                nn.Conv2d(32, channels[0], kernel_size=7, stride=1, padding=3,
                          bias=False),
