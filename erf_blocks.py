...
class UpsamplerBlock(nn.Module):
    def __init__(self, in_channel, out_channel,output_padding=1):
        super().__init__()
        self.conv = nn.ConvTranspose2d(in_channel, out_channel, 3, stride=2, padding=1, output_padding=output_padding, bias=True)
        self.bn = nn.BatchNorm2d(out_channel, eps=1e-3)
        self.relu = nn.ReLU(inplace=True)
